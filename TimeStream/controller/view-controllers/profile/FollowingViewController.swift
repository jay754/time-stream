...
protocol FollowingViewControllerFlowDelegate: BaseViewControllerFlowDelegate {
    func followingGoToUser(vc: FollowingViewController, user: User)
}

class FollowingViewController: BaseViewController, UITableViewDataSource, UITableViewDelegate, UserTableViewCellDelegate {
    
    weak var flowDelegate: FollowingViewControllerFlowDelegate?
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        let user = data[indexPath.row]
        flowDelegate?.followingGoToUser(vc: self, user: user)
    }
    ...
    private func loadFollowees() {
        loading = true
        userService.getFollowees(user: forUser) { (result) in
            self.loading = false
