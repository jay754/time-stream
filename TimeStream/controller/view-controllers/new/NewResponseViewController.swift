//
//  NewResponseViewController.swift
//  TimeStream
//
...
protocol NewResponseViewControllerFlowDelegate: BaseViewControllerFlowDelegate {
    func newResponseGoToPreview(vc: NewResponseViewController, url: URL, message: VideoMessage, conversation: Conversation)
}

class NewResponseViewController: BaseViewController, TopIndicatorViewDelegate, AssetPickerProtocol {
    
    weak var flowDelegate: NewResponseViewControllerFlowDelegate?
    
    var message: VideoMessage!
    var conversation: Conversation!
    
    private struct Constants {
        static let recordTime = 180 // seconds
    }
    ...
    private func goToPreviewWith(url: URL) {
        flowDelegate?.newResponseGoToPreview(vc: self, url: url, message: message, conversation: conversation)
        setInitialState()
    }
