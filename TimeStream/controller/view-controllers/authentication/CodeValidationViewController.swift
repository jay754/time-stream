...
protocol CodeValidationFlowDelegate: BaseViewControllerFlowDelegate {
...
class CodeValidationViewController: BaseViewController, UITextFieldDelegate {
    
    weak var flowDelegate: CodeValidationFlowDelegate?
    ...
    private func validate(code: String) {
        ...
        authService.validateCode(verificationID: validationToken, code: code) { (result) in
            self.loading = false
    ...
    private func fetchCurrentUser() {
        ...
        userService.getCurrentUser { (result) in
            ...
            switch result {
            case .error:
                self.flowDelegate?.codeValidationRegisterUser(vc: self)
    ...
    private func updateCurrentUser(user: User) {
        ...
        loading = true
        userService.updateUser(user: newUser, photo: nil) { (result) in
            self.loading = false
            ...
            case .success(let user):
                Context.current.user = user
                self.flowDelegate?.codeValidationUserAuthenticated(vc: self)
            }
        }
    }
}
