//
//  ChangePhoneNumberValidationViewController.swift
//  TimeStream
//
...
protocol ChangePhoneNumberValidationFlowDelegate: BaseViewControllerFlowDelegate {
    func changePhoneCodeValidationSuccesfullyChanged(vc: ChangePhoneNumberValidationViewController)
}

class ChangePhoneNumberValidationViewController: BaseViewController, UITextFieldDelegate {
    
    weak var flowDelegate: ChangePhoneNumberValidationFlowDelegate?
    ...
    private func validate(code: String) {
        ...
        authService.changePhoneNumber(verificationID: validationToken, verificationCode: code) { (result) in
            self.loading = false
            
            switch result {
            case .error(let error):
                if let error = error as NSError?, error.code == 17025 {
                    self.flowDelegate?.backButtonPressed(from: self)
                    self.showAlert(message: "code.validation.phone.exists".localized)
                    return
                }
    ...
    private func fetchCurrentUser() {
        ...
        userService.getCurrentUser { (result) in
            ...
            case .success(let user):
                ...
                self.flowDelegate?.changePhoneCodeValidationSuccesfullyChanged(vc: self)
            }
        }
    }
}
