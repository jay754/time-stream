...
protocol PhoneFlowDelegate: BaseViewControllerFlowDelegate {
    func phoneFlowHasValidate(vc: PhoneViewController, phoneNumber: String, validationToken: String)
}

class PhoneViewController: BaseViewController,  UIPickerViewDataSource, UIPickerViewDelegate {
    
    weak var flowDelegate: PhoneFlowDelegate?
    ...
    override func viewDidLoad() {
        ...
        let attributedText = NSAttributedString.hyperlinkText(text: "auth.disclaimer".localized, underlined: ["auth.terms".localized, "auth.privacy".localized, "auth.activities".localized], links: [AppLinks.termsAndConditions, AppLinks.privacyPolicy, AppLinks.prohibitedActivities])
    ...
    @IBAction func proceed(_ sender: Any) {
        ...
        authService.validatePhoneNumber(number: number) { (success, id, error) in
            ...
            self.flowDelegate?.phoneFlowHasValidate(vc: self, phoneNumber: number, validationToken: id)
        }
    }
