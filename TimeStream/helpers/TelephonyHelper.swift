...
class TelephonyHelper {
    ...
    static func getRegionCode() -> String {
      guard let regionCodeFromSim = Self.getRegionCodeFromSim() else {
        return NSLocale.current.regionCode?.uppercased() ?? "GB"
      }
    
        return regionCodeFromSim.uppercased()
    }
    
    static func countryName(from countryCode: String) -> String {
        let locale = NSLocale(localeIdentifier: "en_US")
        
        if let name = locale.displayName(forKey: .countryCode, value: countryCode) {
    ...
    static func getCountryCode() -> String? {
      let regionCode = Self.getRegionCode()
